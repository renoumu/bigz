$(function() {
	...
	.done(function(data) {
		...
		for (var i = 0; i <data.notice.length; i++) {
			$('.right-bottom-con').append('<li class="list"><a href="viwe/new.html?id='+target[i].id+'"><span>'+target[i].type+'</span>'+target[i].title+'<span>'+target[i].dates+'</span></a></li>');
			if (i >= 4) {
				break;
			}
			$(".list:last").on('click', function(event) {
				var index = $(this).index();
				
				store.setItem('type"',JSON.stringify(target[index].type));
				store.setItem('title',JSON.stringify(target[index].title));
				store.setItem('content',JSON.stringify(target[index].content));
